...
VOICE_WORKER = Path(__file__).resolve().parents[1] / "backend-rs" / "src" / "voice_worker.rs"
...
class TestVoicePushSource(unittest.TestCase):
    def test_summary_prompts_cover_final_and_narration_targets(self) -> None:
        source = VOICE_WORKER.read_text(encoding="utf-8")
    ...
    def test_notification_text_is_canonical_backend_field(self) -> None:
        ...
        self.assertIn('"notification_text": notification_text', source)
