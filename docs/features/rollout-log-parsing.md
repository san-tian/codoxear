...
- Codex `event_msg` lifecycle records feed idle detection: `task_started` marks the session busy, while `task_complete`, `turn_complete`, `turn_aborted`, and `agent_message` records with `phase: "final_answer"` mark the current turn idle/closed. `response_item` assistant messages with `phase: "final_answer"` are also treated as turn-ending even when the CLI omits an explicit `end_turn` flag.
...
- Rust `messages/live` normalizes tool calls/results, `ask_user`, `update_plan`, `codoxear_display`, and `diag.tool_names` / `diag.last_tool` plus `meta_delta.thinking|tool|system`. Live parsing also emits final `event_msg.agent_message` text and sets `turn_end` for final-answer records so Nova can clear `working` without waiting for a separate `task_complete` record. The `messages/tail` response carries the latest turn boundary as well, so opening or refreshing an already completed session clears awaiting-reply state even when the visible tail has only a user row plus a lifecycle completion record.
