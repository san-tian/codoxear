...
- Shared-session viewers use share-scoped queue routes for sessions in the share set; those routes reuse the same persisted `session_queues.json` items as the owner queue routes, so queued messages are shared between the main session view, share view, schedules, and the daemon queue-drain worker.
